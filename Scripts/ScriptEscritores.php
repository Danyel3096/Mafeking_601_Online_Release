...
$(document).ready(function() {
  comentariosNoticia();
	$.ajax({
    	...
    	data:{accion: "informacion-ramas"},
    	success:function(datos) {
        var objeto = JSON.parse(datos);
        var lista_ordenada = "";
        var lineas = objeto[2]['Ley'].split('\n');
        for(i = 0; i < lineas.length; i++){
          lista_ordenada  += '<strong>'+(i+1)+')</strong> '+lineas[i]+'<br />';
        }
        //$("#imagen-grupo").html(objeto[0]['Imagen']);
        $("#historia-grupo").html(objeto[0]['Historia'].replace(/\n/g,"<br />"));
        $("#titulo-jefatura").html(objeto[0]['Nombre']);
        $("#foto-historica-jefatura").prop("src", "Archivos/Imagenes/Ramas/Jefatura/"+objeto[0]['Foto_historica']);
        $("#titulo-consejo").html(objeto[1]['Nombre']);
        $("#foto-historica-consejo").prop("src", "Archivos/Imagenes/Ramas/Consejo/"+objeto[1]['Foto_historica']);
        $("#titulo-manada").html(objeto[5]['Nombre']);
        $("#foto-historica-manada").prop("src", "Archivos/Imagenes/Ramas/Manada/"+objeto[5]['Foto_historica']);
        $("#titulo-tropa").html(objeto[4]['Nombre']);
        $("#foto-historica-tropa").prop("src", "Archivos/Imagenes/Ramas/Tropa/"+objeto[4]['Foto_historica']);
        $("#titulo-comunidad").html(objeto[3]['Nombre']);
        $("#foto-historica-comunidad").prop("src", "Archivos/Imagenes/Ramas/Comunidad/"+objeto[3]['Foto_historica']);
        $("#titulo-clan").html(objeto[2]['Nombre']);
        $("#foto-historica-clan").prop("src", "Archivos/Imagenes/Ramas/Clan/"+objeto[2]['Foto_historica']);

    		$("#titulo-clan").html(objeto[2]['Nombre']);
    		$("#historia-clan").html(objeto[2]['Historia'].replace(/\n/g,"<br />"));
    		$("#ley-clan").html(lista_ordenada);
    		$("#promesa-clan").html(objeto[2]['Promesa']);
    		$("#lema-clan").html('«'+objeto[2]['Lema']+'»');
        $("#oracion-clan").html(objeto[2]['Oracion'].replace(/\n/g,"<br />"));
        $("#clan-foto-historica").prop("src", "../Archivos/Imagenes/Ramas/Clan/"+objeto[2]['Foto_historica']);

        $("#titulo-comunidad").html(objeto[3]['Nombre']);
        $("#historia-comunidad").html(objeto[3]['Historia'].replace(/\n/g,"<br />"));
        $("#promesa-comunidad").html(objeto[3]['Promesa']);
        $("#lema-comunidad").html('«'+objeto[3]['Lema']+'»');

        $("#titulo-tropa").html(objeto[4]['Nombre']);
        $("#historia-tropa").html(objeto[4]['Historia'].replace(/\n/g,"<br />"));

        $("#titulo-manada").html(objeto[5]['Nombre']);
        $("#historia-manada").html(objeto[5]['Historia'].replace(/\n/g,"<br />"));

      	}
  	});
    participacionesRamaGrupo();
});
...
function participacionesRamaGrupo() {
  var id_rama = "1";
  var accion = "linea-tiempo";
  ...
  $.ajax({
    ...
    data:cadena,
    success:function(datos){
      var arreglo_datos = JSON.parse(datos);
      var options_timeline = {language: "es", script_path: "https://cdn.jsdelivr.net/npm/timelinejs3@3.4.0/source/js/language/"};
      var linea_tiempo_grupo = new TL.Timeline('linea-tiempo-grupo', arreglo_datos, options_timeline);
    }
  });
}
function participacionesRamaClan() {
  var id_rama = "3";
  var accion = "linea-tiempo";
  var cadena = "Id-rama="+id_rama+"&accion="+accion;
  $.ajax({
    ...
    data:cadena,
    success:function(datos){
      var linea_tiempo_clan = new TL.Timeline('linea-tiempo-clan', datos);
    }
  });
}
function participacionesRamaComunidad() {
  var id_rama = "4";
  var accion = "linea-tiempo";
  var cadena = "Id-rama="+id_rama+"&accion="+accion;
  $.ajax({
    ...
    data:cadena,
    success:function(datos){
      var linea_tiempo_comunidad = new TL.Timeline('linea-tiempo-comunidad', datos);
    }
  });
}
function participacionesRamaTropa() {
  var id_rama = "5";
  var accion = "linea-tiempo";
  var cadena = "Id-rama="+id_rama+"&accion="+accion;
  $.ajax({
    ...
    data:cadena,
    success:function(datos){
      var linea_tiempo_tropa = new TL.Timeline('linea-tiempo-tropa', datos);
    }
  });
}
function participacionesRamaManada() {
  var id_rama = "6";
  var accion = "linea-tiempo";
  var cadena = "Id-rama="+id_rama+"&accion="+accion;
  $.ajax({
    ...
    data:cadena,
    success:function(datos){
      var linea_tiempo_manada = new TL.Timeline('linea-tiempo-manada', datos);
    }
  });
}
var cadena_json = {
    "title": {
        ...
        "text": {
          "headline": "Eventos en los que ha participado el Clan 1 San Pablo<br/> 1963 - 2012",
